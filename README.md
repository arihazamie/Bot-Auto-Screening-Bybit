...
```bash
# 1. Clone and install
git clone https://github.com/arihazamie/Bot-Auto-Screening-OKX.git
cd Bot-Auto-Screening-OKX
python3 -m venv venv
...
#   (OKX keys are optional for paper mode)
...
2. Pick any name and a unique username ending in `bot` (e.g. `myokx_bot`).
...
### 4. (Optional) OKX API key

The bot is signal-only and pulls all candle data from public endpoints, so an OKX API key is **not required**. Leave the placeholders in `config.json`.
...
For paper mode you can leave `okx_key` and `okx_secret` as placeholders — the bot only uses public market data in paper mode.
...
### 6. (Optional) If you're in a region where OKX is blocked

You'll see `403 Forbidden` or DNS errors when the bot tries to fetch prices. Use a VPN with an exit node in a country where OKX is accessible (e.g. Singapore, Japan, Germany), then start the bot.
...
```ini
# /etc/systemd/system/okx-bot.service
[Unit]
Description=OKX Screening Bot
After=network.target

[Service]
Type=simple
User=youruser
WorkingDirectory=/home/youruser/Bot-Auto-Screening-OKX
ExecStart=/home/youruser/Bot-Auto-Screening-OKX/venv/bin/python main.py
Restart=on-failure
RestartSec=10
...
Then `sudo systemctl enable --now okx-bot`.
...
| `use_max_leverage`        | `true`            | If true, use OKX's per-coin max (capped by `max_leverage_cap`)      |
...
The biggest section. Knobs that change _what counts as a signal_ and _what the resulting entry/SL/TP looks like_.
...
When the BTC 1h regime classification changes between scans, the bot sends a Telegram alert (e.g. _"BTC: TREND_BULL → ANOMALY — new entries auto-paused"_). The previous regime is persisted in `state_store` so transitions are detected across restarts.
...
| Key                     | Default | What it does                                                                                                  |
| ----------------------- | ------- | ------------------------------------------------------------------------------------------------------------- |
| `show_regime_breakdown` | `true`  | Append a _By Regime_ section listing trade count + W/L + WR + PnL per regime, plus best/worst regime call-out |

### Patterns NOT exposed as config

Some pattern definitions are _part of the pattern itself_ and would corrupt the detector if tweaked:
...
### `403 Forbidden` or `Connection refused` when fetching OKX data

You're in a region where OKX is blocked. Use a VPN with an exit node in a country where OKX is accessible.
...
Watchlist refresh failed for >36 hours (OKX rate-limit, network, or VPN issue). The bot will try again on the next scan cycle. If it persists, delete `data/watchlist.json` to force a fresh fetch.
...
└── modules/
    ...
    ├── exchange.py               # OKX client (ccxt) — retry, leverage cache
