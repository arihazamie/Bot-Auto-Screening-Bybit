...
| 13        | Keamanan API & Config    |    10/10    | API key di `config.json` (tidak hardcode) ✅, `.gitignore` cover `config.json` & `data/` ✅, placeholder validation ✅, API key hanya dimuat saat `auto_trade=true` ✅, validasi explicit jika key kosong/placeholder ✅, `_authorized()` fix direct comparison ✅ |
