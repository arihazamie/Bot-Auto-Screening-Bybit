...
from modules.database import init_db, get_active_signals, save_signal_to_db, set_state
...
from modules.telegram_commands import start_command_handler
...
logger.info(f"🔭 Scan dimulai | Mode: {mode_label}")
    ...
    set_state("btc_bias", btc_bias)
    set_state("last_scan_at", datetime.now().strftime("%H:%M"))
...
init_db()
    ...
    print("🎮 Telegram Command Handler — AUTO START")
    print("   Commands: /help /status /balance /positions /pending /trades /report")
    print()
